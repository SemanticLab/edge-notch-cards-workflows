...
def ask_gemini_match(card_data, wikidata_qid, wikidata_label, wikidata_props):
    """Ask Gemini whether the Wikidata entity matches the card person."""
...
def process_card(card_path, test_mode=False):
    """Process a single card: search Wikidata, get data, ask Gemini. Returns list of candidates."""
    ...
    # Step 1: Search Wikidata for matching people (with fallback)
    ...
    for match in matches:
        ...
        gemini_result = ask_gemini_match(card_data, qid, label, props)
...
def main():
    parser = argparse.ArgumentParser(description="Research front cards against Wikidata")
    ...
    for i, card_path in enumerate(front_files, 1):
        ...
        # Skip if already has wikidata_candidates
        if "wikidata_candidates" in card_data:
            already_done += 1
            continue
        ...
        try:
            ...
            card_data["wikidata_candidates"] = candidates
