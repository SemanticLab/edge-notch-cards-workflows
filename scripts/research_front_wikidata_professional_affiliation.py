...
def search_wikidata_entity(name, limit=None):
    """Search Wikidata for any entity matching the given name (no Q5 restriction). Returns list of {qid, label, description}."""
  ...
  SERVICE wikibase:label {{ bd:serviceParam wikibase:language "en". }}
    ...
    for row in results.get("results", {}).get("bindings", []):
        ...
        items.append({"qid": qid, "label": label, "description": description})
    return items


def split_multi_org(name):
    """Split strings with multiple orgs joined by & or 'and'.
    'New York University & City College of New York' -> ['New York University', 'City College of New York']
    Only splits on ' & ' or ' and ' when they appear to join separate org names (not part of a single name like 'Bolt Beranek and Newman').
    """
    # Split on ' & ' (almost always joining separate orgs)
    if " & " in name:
...
def strip_address(name):
    ...
    Only strips if what follows the comma looks like an address (starts with a number, or is a known city/state pattern).
    """
    # Match comma followed by a space and a digit (street number)
    m = re.match(r'^(.+?),\s+\d', name)
    if m:
        return m.group(1).strip()
    # Match comma followed by known geographic indicators
...
def strip_parenthetical(name):
    ...
    'The Danelectro Corporation (Subsidiary of Music Corporation of America Inc)' -> 'The Danelectro Corporation'
    """
...
def strip_corporate_suffix(name):
    ...
    'Just Plastics Incorporated' -> 'Just Plastics'
    """
...
def generate_search_variants(org_name):
    ...
    if paren_match:
        ...
        # Only search abbreviation if it looks like a real name/acronym (not "Subsidiary of...")
    ...
    if len(parts) > 1:
        for part in parts:
            ...
            add(simplified, f"split + simplified")

    return variants


def search_with_fallback(org_name, test_mode=False):
    """Search Wikidata with progressively simplified org name variants.
    ...
    """
    # Check if this is a multi-org string
    parts = split_multi_org(org_name)
    if len(parts) > 1:
        all_matches = []
        seen_qids = set()
...
def _search_single_org(org_name, test_mode=False):
    ...
    for variant, desc in variants:
        if test_mode:
            print(f'  Searching Wikidata for "{variant}" ({desc})...')
        matches = search_wikidata_entity(variant, limit=10)
        if matches:
...
def get_entity_data(qid):
    """Get all non-external-id properties for a Wikidata entity. Returns list of {property, value}."""
...
def ask_gemini_match(card_data, org_name, wikidata_qid, wikidata_label, wikidata_description, wikidata_props):
    """Ask Gemini whether the Wikidata entity matches the employer/organization from the card."""
    prompt = f"""Compare these two records and determine if the Wikidata entity is the same organization as the employer listed on the card.
...
## Wikidata Entity: {wikidata_qid} ({wikidata_label}) - {wikidata_description}
{json.dumps(wikidata_props, indent=2)}

Is this Wikidata entity the same organization as "{org_name}"? Consider the name, type of organization, location, industry, and any other overlapping fields.
Return JSON with:
...
def process_card(card_path, test_mode=False):
    """Process a single card: search Wikidata for the employer, get data, ask Gemini. Returns list of candidates."""
    ...
    # Step 1: Search Wikidata with fallback simplification
    matches, searched_variant = search_with_fallback(org_name, test_mode=test_mode)

    if not matches:
        if test_mode:
            print(f"  No Wikidata results found for any variant")
        return []
...
def main():
    parser = argparse.ArgumentParser(description="Research front card employer/organization against Wikidata")
    ...
    for i, card_path in enumerate(front_files, 1):
        ...
        # Skip if already has wikidata_org_candidates
        if "wikidata_org_candidates" in card_data:
            already_done += 1
            continue
        ...
        try:
            ...
            card_data["wikidata_org_candidates"] = candidates
